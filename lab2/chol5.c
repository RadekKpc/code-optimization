        ...
        for (i = j; i < n; i++) {
            ...
            for (k = 0; k < j; ) {
                if(k < max(j - 16, 0)){
                    ...
                    tmp8 - _mm_loadu_pd(A+IDX(i, k, n));
                    tmp9 - _mm_loadu_pd(A+IDX(j, k, n));
                    tmp10 - _mm_loadu_pd(A+IDX(i, k + 2, n));
                    tmp11 - _mm_loadu_pd(A+IDX(j, k + 2, n));
                    tmp12 - _mm_loadu_pd(A+IDX(i, k + 4, n));
                    tmp13 - _mm_loadu_pd(A+IDX(j, k + 4, n));
                    tmp14 - _mm_loadu_pd(A+IDX(i, k + 6, n));
                    tmp15 - _mm_loadu_pd(A+IDX(j, k + 6, n));
